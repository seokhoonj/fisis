...
:class:`~fisis.types.Table` (its ``rows`` for the values, ``columns`` for each
...
from .types import Lang, Table, Term


class _BankCompanyView(CompanyView):
    """A bank company, with its headline statistics reachable by name.

    Each method returns a :class:`~fisis.types.Table`; its ``columns`` carry the
